...
Future<List<GeneVariant>> loadGeneVariants() async {
  var url = Uri.parse('http://127.0.0.1:8000/variations/');
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonList is List) {
        List<GeneVariant> results =
            jsonList.map((json) => GeneVariant.fromJson(json)).toList();
      ...
      print('Error loading gene variants: ${response.statusCode}');
